public class DevelopmentSuggestionHandler implements Handler {
    ...
    public void handleMessage(Message n) {
        if (n.messageType.toString().equals(Type.DEVELOPMENT_SUGGESTION.toString())) {
            System.out.println(n.message + " Has been handled by development suggestion");
        } else {
            handlers[3].handleMessage(n);
        }
    }
