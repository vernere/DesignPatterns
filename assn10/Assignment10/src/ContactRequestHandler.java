public class ContactRequestHandler implements Handler {
    ...
    public void handleMessage(Message n) {
        if (n.messageType.toString().equals(Type.CONTACT_REQUEST.toString())) {
            System.out.println(n.message + " Has been handled by contact request");
        } else {
            handlers[2].handleMessage(n);
        }
    }
